...
import ApiClient from "../../../services/API"; // لو عندك كلاس ApiClient
...
export default function MonthlyTarget() {
  ...
  //     try {
  //       const res = await ApiClient.get("/dashboard/monthly-target"); // endpoint تجريبي
  //       const data = res.data;
    ...
    <div className="rounded-2xl border border-gray-200 bg-gray-100">
      <div className="px-5 pt-5 bg-white shadow-default rounded-2xl pb-11 sm:px-6 sm:pt-6">
        ...
        <p className="mx-auto mt-10 w-full max-w-[380px] text-center text-sm text-gray-500 sm:text-base">
          {targetData.message ||
            "You earn more this month. Keep up your good work!"}
        </p>
      </div>

      <div className="flex items-center justify-center gap-5 px-6 py-3.5 sm:gap-8 sm:py-5">
        {[
          { label: "Target", value: targetData.target, color: "#D92D20" },
          { label: "Revenue", value: targetData.revenue, color: "#039855" },
          { label: "Today", value: targetData.today, color: "#039855" },
        ].map((item, i) => (
